...
> 项目级 override：用户在书目录 `books/<id>/genres/<id>.md` 自定义同名文件时（与 inkos 的 BUILTIN_GENRES_DIR / projectRoot 双层 lookup 一致），优先读项目级。SKILL 当前简化版只读 `templates/genres/`，需要项目级 override 时，约定把项目级文件放到 `books/<id>/genres/<id>.md`，并在加载时先尝试项目级。
