...
- 写入 `story/runtime/settlement.md` 包含两个 TAG 块（先 POST_SETTLEMENT，后 RUNTIME_STATE_DELTA）
- DELTA JSON 必须能被 `scripts/apply_delta.py` 通过 `jsonschema` 校验
...
- **JSON 解析失败** → 重试 1 次（附"上次输出无法解析为 JSON，请只输出 RUNTIME_STATE_DELTA 块的 JSON"feedback），仍失败抛错。
- **schema 校验失败**（apply_delta 拒绝）→ 把 jsonschema 的具体错误回喂给 Claude，重试 1 次；仍失败抛错给用户。
