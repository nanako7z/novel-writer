...
12. **`SectionReplaceOp.newContent` 不要包含任何 H2/H3 行**——既不要重复 anchor 自己那一行，也不要夹其他 `## ` / `### ` 行：`anchor` 字段已经声明了节标题，`newContent` 只是节内容（H2 标题之下的正文，纯 inline：`-` 列表 / 段落 / `**bold**` 子标题 / 表格）。
    - **`currentFocus` 专属硬约束**：`story/current_focus.md` 的合法 anchor 只有 `## Active Focus` 和 `## 暂缓项` 两个（见 [templates/story/current_focus.md](../../templates/story/current_focus.md)）。Settler **不允许**通过 newContent 引入任何新 H2 section（如 `## 近3章焦点` / `## 历史焦点` / `## Recent Chapters`）——这种结构变更属于产品语义层面，不是状态结算的职责。要追加"近期章节焦点"之类的回顾信息，请放进现有 `## Active Focus` 的列表项里，或写成 `**近 3 章**：第 4-6 章兑现了 X` 这种 bold 行，不要起新 H2。
    - 错误示例（**不要这样**）：
      ```jsonc
      ...
      { "op": "replace_section", "anchor": "## Active Focus",
        "newContent": "- 当前章次：12\n## 近3章焦点\n第10章: ...\n" }
      // ❌ 嵌入了新 H2；apply_delta 会把它降级为 #### 并记 softFix
      ```
    - 正确示例：
      ```jsonc
      { "op": "replace_section", "anchor": "## Active Focus",
        "newContent": "- 当前章次：12\n- 接下来 1-3 章要做的事：\n  - 第 13 章：..." }  // ✅
      ```
    - **运行时兜底**：apply_delta 会自动剥离首行的重复 anchor、把中部 H2/H3 降级为 H4/H5，并各记一笔 softFix（`stripped_leading_anchor` / `demoted_embedded_h2_lines`）。这是**兜底**不是免检——反复犯会让 `doc_changes.log` 膨胀、`current_focus.md` 视觉结构走形，`docops_drift` 也会反复报。目标是 canonical 输出。
...
- 写入 `story/runtime/settlement.md`，含 `=== POST_SETTLEMENT ===` + `=== RUNTIME_STATE_DELTA ===`（```json``` 围栏）
- DELTA 必须通过 [schemas/runtime-state-delta.md](../schemas/runtime-state-delta.md) 校验
- 调用：`python scripts/apply_delta.py --book <book> --delta settlement.md --input-mode raw`
- 调试：`python scripts/settler_parse.py --mode raw --input settlement.md --out delta.json`（不写盘）
...
| 状态校验矛盾（hook 同时 resolve + upsert） | — | 附 `validationFeedback` 重跑（**不改正文**只修 truth file） | 1 次 |
| 数值不平（数值题材） | — | 拒 delta 重跑；仍不平交 Auditor | 1 次 |

**每章总重试上限 2 次。** 原子性见 invariant #1（失败 = 真理文件回到应用前状态；成功后重跑 = no-op，靠 `manifest.appliedDeltaHash`）。重跑必须注入 feedback（invariant #5）。

## 注意事项

- **不删除无关 hook**：上一章的 hook 与本章无关时不要碰它
- **第 1 章特殊**：初始 hooks.json 里可能含 architect 预生成 `startChapter=0` 行——只更新本章实际触及的
- **`enableFullCastTracking`**：开了就在 POST_SETTLEMENT 加"出场清单 / 关系变动 / 提及但未出场"三段
