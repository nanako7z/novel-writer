...
3. **写 RUNTIME_STATE_DELTA**：JSON 增量。严格按 schema：
   ...
   - `subplotOps` / `emotionalArcOps` / `characterMatrixOps`: 增量操作数组（已升级为 upsert 语义，由 doc_ops 内部统一处理）
     - **`emotionalArcOps` 每条必须四列齐写**：`character`, `chapter`, `emotionalState`, `triggerEvent`, `intensity` (1-10 整数), `arcDirection` (rising / falling / stable / turning)。缺/空任一列会被 schema 拒绝，触发 Settler 重跑。
