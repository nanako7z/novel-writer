...
**当前规模**：14 phase 文档（含编排）+ 23 篇 references 设定 / 算法 / 治理文档 + 7 份数据契约 schema + 42 个 Python 脚本（标准库，跑 `doctor.py` 自检全 ok）+ 15 题材 profile + 6 站 radar adapter（番茄 SDK / 起点 / 飞卢 / 晋江 / 纵横 / SF 轻小说）。
...
```
看下当前番茄玄幻榜什么火，给我 3 个开书建议
# 或：现在网文市场什么火 / radar
```

Claude 会：

1. 跑 `radar_fetch.py scan --sites all --genre xianxia`，从 6 家站（番茄 / 起点 / 飞卢 / 晋江 / 纵横 / SF 轻小说）抓榜
2. 6h 内已有缓存就直接用；否则 std-lib 现抓（番茄走 SDK API，余下站静态 HTML 正则）
3. 反爬命中 / 模板漂移的站，自动接力调一次 WebFetch 兜底
4. 还失败的让你贴一份榜单
5. 进 [phase 01 radar](references/phases/01-radar.md) prompt 出 3-5 条带 confidence 的推荐

详细站点对照、可靠性档位、题材映射见 [references/radar-sources.md](references/radar-sources.md)。
...
| radar + radar-source | [phase 01](references/phases/01-radar.md) + `scripts/radar_fetch.py` + 6 站 adapter（[radar-sources.md](references/radar-sources.md)）；3 档递进兜底 std-lib → WebFetch → user-paste |
...
| 平台 Radar | 实时抓取番茄+起点 2 家 | `radar_fetch.py` 抓 6 家（番茄 SDK / 起点 / 飞卢 / 晋江 / 纵横 / SF 轻小说），3 档递进兜底 |
